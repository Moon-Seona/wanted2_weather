...
class ProblemDataset(Dataset):
    def __init__(self, file, use_tokenizer, stop_words, val):
        ...
        # through each label.
        data = pd.read_csv(file, engine='python').fillna('')
        self.texts = data['과제명'] + ' [SEP] ' + data['요약문_연구목표'] + ' [SEP] ' + data['요약문_기대효과'] + ' [SEP] ' \
                     + data['요약문_한글키워드'] + ' [SEP] ' + data['요약문_연구내용']
...
class L1Trainer():
    def __init__(self, batch_size, max_length):
        ...
        data = path + 'train4.csv'
        test_data = path + 'test2.csv'
